...
EMBEDDED_TELEGRAM_TOKEN = ""  # Вставьте токен бота сюда, чтобы не настраивать в UI
EMBEDDED_TELEGRAM_CHAT_ID = ""  # Вставьте chat_id сюда
...
def parse_form_data(handler: http.server.BaseHTTPRequestHandler) -> Tuple[Dict[str, str], Dict[str, bytes]]:
...
class TelegramSync:
    ...
    def sync_once(self) -> None:
        ...
        token = cfg.get("telegram_token", "").strip() or EMBEDDED_TELEGRAM_TOKEN.strip()
        chat_id = str(cfg.get("telegram_chat_id", "")).strip() or EMBEDDED_TELEGRAM_CHAT_ID.strip()
...
<div class='container'>
  ...
  <div class='card'>
    ...
    <form method='post' action='/upload_zip' enctype='multipart/form-data'>
      <p>Выберите ZIP-файл с сайтом. В архиве должен быть index.html.</p>
      <input name='title' placeholder='Название сайта' required size='30'/>
      <input name='domain' placeholder='my-zip-site' required size='30'/>
      <input name='description' placeholder='Описание' size='60'/>
      <p><input type='file' name='zip_file' accept='.zip,application/zip' required/></p>
...
def settings_page(message: str = "") -> bytes:
    ...
    token_preview = cfg.get("telegram_token", "") or ("[embedded]" if EMBEDDED_TELEGRAM_TOKEN else "")
    chat_preview = str(cfg.get("telegram_chat_id", "") or ("[embedded]" if EMBEDDED_TELEGRAM_CHAT_ID else ""))
    msg = f"<div class='card'>{html.escape(message)}</div>" if message else ""
    body = f"""
<header><div class='row'><a href='/'><button>← Главная</button></a></div></header>
<div class='container'>
  ...
  <div class='card'>
    <form method='post' action='/save_settings'>
      <p>Telegram Bot Token: <input name='telegram_token' value='{html.escape(token_preview)}' size='70'/></p>
      <p>Telegram Chat ID: <input name='telegram_chat_id' value='{html.escape(chat_preview)}' size='30'/></p>
      <p><button type='submit'>Сохранить</button></p>
    </form>
    <small>Если поля пустые, используются EMBEDDED_TELEGRAM_TOKEN/CHAT_ID из кода.</small>
  </div>
</div>
"""
...
class MaxnetHandler(http.server.SimpleHTTPRequestHandler):
    ...
    def do_POST(self) -> None:  # noqa: N802
        ...
        if parsed.path == "/publish":
            ...
            domain = ensure_domain_suffix(sanitize_domain(fields.get("domain", "")))
            desc = fields.get("description", "").strip()
            index_html = fields.get("index_html", "").strip() or "<h1>Hello MaxNet</h1>"
        ...
        if parsed.path == "/upload_zip":
            title = fields.get("title", "").strip() or "ZIP Site"
            domain = ensure_domain_suffix(sanitize_domain(fields.get("domain", "")))
            desc = fields.get("description", "").strip()
            raw = files.get("zip_file", b"")
            if not raw:
                self._send_html(editor_page("Ошибка: выберите ZIP файл"))
                return
        ...
        if parsed.path == "/save_settings":
            token = fields.get("telegram_token", "").strip()
            chat_id = fields.get("telegram_chat_id", "").strip()
            if token == "[embedded]":
                token = ""
            if chat_id == "[embedded]":
                chat_id = ""
